...
for crisis in crises_cols:
    ...
    # Calculate the mean of m pairwise tests
    true_pairwise_mean = np.mean([pairwise_test(FRA_sample, GBR_sample, n)
                                  for _ in range(m)])

    # Resample our data using a multinomial distribution
    gbr_p_values = np.array(GBR_sample) / sum(GBR_sample)
    fra_p_values = np.array(FRA_sample) / sum(FRA_sample)

    # TODO: Not sure if it should be length of sum
    gbr_resamples = np.random.multinomial(sum(GBR_sample), gbr_p_values,
                                          size=50)
    fra_resamples = np.random.multinomial(sum(FRA_sample), fra_p_values,
                                          size=50)
    ...
    resampled_pairwise_tests = [np.mean([pairwise_test(f, g, n)
                                         for _ in range(m)])
                                for f, g in zipped_resamples]

    # Plotting stuff
    plt.title(crisis)
    plt.xlabel('Percentage FRA > GBR')
    plt.ylabel('Count')
    plt.hist(binomial_samples, bins=int(np.sqrt(m)))
    ...
    # Plot each of our resampled means in magenta and low opaqueness
