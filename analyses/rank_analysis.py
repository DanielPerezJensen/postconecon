...
import random
from master import get_colonist
...
for crisis in crises_cols:
    ...
    true_p_value = mannwhitneyu(fra_sample, gbr_sample).pvalue
    ...
    resamples = np.random.multinomial(sum(full_sample),
                                      full_sample_probs, size=n)

    # Run our pairwise test on each of the n new samples
    resampled_mannwhitneyu = [mannwhitneyu(r[:len(fra_sample)], r[len(fra_sample):]).pvalue
                              for r in resamples]

    # Plotting stuff
    plt.figure()
    plt.title(f'p-values of {crisis.replace("_", " ")}')
    plt.xlabel('p-value')
    plt.ylabel('Count')
